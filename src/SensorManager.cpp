...
void SensorManager::beginSensors(const std::vector<SensorConfig>& enabledNormalSensors,
    ...
    // // Inicializar RTD solo si está habilitado en la configuración - Comentado porque Adafruit_MAX31865 está deshabilitado
    // if (rtdSensorEnabled) {
    ...
    //     //     if (sensor.type == RTD && sensor.enable) {
    //     //         std::string currentSensorId = sensor.sensorId;
    //     //         setSensorInitialized(currentSensorId, true); // Marcar como inicializado aunque esté comentado
    //     //     }
    //     // }
    // }
...
float SensorManager::readSensorValue(const SensorConfig &cfg, SensorReading &reading) {
    ...
    switch (cfg.type) {
        ...
        case RTD: // Comentado porque Adafruit_MAX31865 está deshabilitado
            // {
            ...
            //     //     reading.value = temp;
            //     // }
            //     reading.value = NAN; // Asignar NAN ya que el sensor está deshabilitado
            // }
            break; // Mantenemos el break para la estructura del switch
